...
mapfile -t dirs_without_deployment < <(find "$PWD"/helmCharts/*/charts/*/templates -type d -exec bash -c 'shopt -s nullglob; files=("$1"/deploy*.yaml); [[ ${#files[@]} -eq 0 ]]' _ {} \; -print)

# Mapeia os diretórios dentro da pasta templates, alguns diretórios não possuem a pasta charts.
mapfile -t template_dirs_without_deployment < <(find "$PWD"/helmCharts/*/templates -type d -exec bash -c 'shopt -s nullglob; files=("$1"/deploy*.yaml); [[ ${#files[@]} -eq 0 ]]' _ {} \; -print)
